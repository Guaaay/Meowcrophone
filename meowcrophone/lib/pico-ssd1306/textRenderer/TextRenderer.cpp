...
namespace pico_ssd1306 {
    ...
    void drawTextDouble(pico_ssd1306::SSD1306 *ssd1306, const unsigned char *font, const char *text, uint8_t anchor_x,
                  uint8_t anchor_y, WriteMode mode, Rotation rotation) {
        if(!ssd1306 || !font || !text) return;
        ...
        while (text[n] != '\0') {
            switch (rotation) {
                case Rotation::deg0:
                    drawCharDouble(ssd1306, font, text[n], anchor_x + (n * font_width), anchor_y, mode, rotation);
                    break;
                case Rotation::deg90:
                    drawCharDouble(ssd1306, font, text[n], anchor_x, anchor_y + (n * font_width), mode, rotation);
                    break;
            }
    ...
    void drawCharDouble(pico_ssd1306::SSD1306 *ssd1306, const unsigned char *font, char c, uint8_t anchor_x, uint8_t anchor_y,
                  WriteMode mode, Rotation rotation) {
        if(!ssd1306 || !font || c < 32) return;
        ...
        bool repeat = true;
        for (uint8_t x = 0; x < font_width; x++) {
            for (uint8_t y = 0; y < font_height*2; y++) {
                if (font[seek] >> b_seek & 0b00000001) {
                    switch (rotation) {
                        case Rotation::deg0:
                            ssd1306->setPixel(x + anchor_x, y + anchor_y, mode);

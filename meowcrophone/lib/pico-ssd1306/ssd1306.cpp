...
namespace pico_ssd1306 {
    ...
    void SSD1306::addBitmapImageHalf(int16_t anchorX, int16_t anchorY, uint8_t image_width, uint8_t image_height,
                            const uint8_t *image,
                            WriteMode mode) {
        uint8_t byte;
        // goes over every single bit in image and sets pixel data on its coordinates
        ...
        int newX = anchorX;
        int newY = anchorY;
        for (uint8_t y = 0; y < image_height; y+=1) {
            for (uint8_t x = 0; x < new_width / 4; x+=1) {
                byte = image[y * (new_width / 4) + x];
                for (uint8_t z = 0; z < 8; z+=2) {
                    if ((byte >> (7 - z)) & 1) {
                        this->setPixel(x * 4 + z/2 + anchorX, y/2 + anchorY, mode);
                    }
                }
            }
        }
